...
class EcmultTests: XCTestCase {
    ...
    func testRandomPoint() {
        ...
        let a = Secp256k1Group(
            x: Secp256k1Field(words64:[0x6d986544_57ff52b8, 0xcf1b8126_5b802a5b, 0xa97f9263_b1e88044, 0x93351325_91bc450a].reversed()),
    ...
    func testEmultChain() {
        ...
        var ae = Secp256k1Scalar.one
        var ge = Secp256k1Scalar.zero
    ...
    func testPointTimesOrder() {
        ...
        for _ in 0..<500 {
            if let point = Secp256k1Group(x: x, odd: true) {
                ...
                let res3 = ecmult.gen(point: point, pn: Secp256k1Scalar.zero, gn: Secp256k1Scalar.zero)
                XCTAssertTrue(res3.isInfinity)
                var res4 = ecmult.gen(point: point, pn: Secp256k1Scalar.one, gn: Secp256k1Scalar.zero)
                res4.normalizeJ()
                XCTAssertEqual(res4, point)
                var res5 = ecmult.gen(point: point, pn: Secp256k1Scalar.zero, gn: Secp256k1Scalar.one)
                res5.normalizeJ()
                XCTAssertEqual(res5, Secp256k1Ecmult.g)
            }
            x = Secp256k1Field.sqr(x)
        }
        XCTAssertEqual(x, xr)
